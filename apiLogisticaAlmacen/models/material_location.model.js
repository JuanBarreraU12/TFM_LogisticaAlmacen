...
/*const getAll = (warehouseId) => {
    return executeQuery('SELECT ml.id, m.name AS material, m.price, m.sku, l.description AS location, ml.stock FROM materials_locations ml JOIN materials m ON ml.materials_id = m.id JOIN locations l ON ml.locations_id = l.id JOIN warehouses w ON l.warehouses_id = w.id WHERE w.id = ?', [warehouseId]);
}

const getByLocationWithPages = (warehouseId, locationId, page, limit) => {
    return executeQuery('SELECT ml.id, m.name AS material, m.price, m.sku, l.description AS location, ml.stock FROM materials_locations ml JOIN materials m ON ml.materials_id = m.id JOIN locations l ON ml.locations_id = l.id JOIN warehouses w ON l.warehouses_id = w.id WHERE w.id = ? AND l.id = ? LIMIT ? OFFSET ?', [warehouseId, locationId, limit, (page - 1) * 10]);
}
